...
export class ListProduitsComponentComponent {
 ...
 {idProduit:5,code:"F147852P",libelle:"Produit1", prixUnitaire:70, tauxTVA:0.02},
  ...
  SearchProduct(serachTerm:string):void{
    if(!serachTerm){
      this.listProduits= this.listProduitsOriginal;}
        ...
        this.listProduits=this.listProduitsOriginal.filter((produit)=>produit.code.toLowerCase().includes(serachTerm.toLowerCase()));
      }
  }
  OpenAddProduct(){
  ...
  onProductAdded(newProduct: any) {
    this.listProduits.push(newProduct); // Ajoutez le nouveau produit à la liste
  }
